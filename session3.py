...
SKILLS_S3 = {
    ...
    "s3_step2": _load_skill("step02_qready.md"),
    "s3_step3":  _load_skill("step03_questionnaire_discussion.md"),
    "s3_step4":  _load_skill("step04_checkin.md"),
    "s3_step5":  _load_skill("step05_narrative_feedback.md"),
    "s3_step6":  _load_skill("step06_writing_instructions.md"),
    "s3_step7":  _load_skill("step07_suds_pre.md"),
    "s3_step8":  _load_skill("step08_writing_task.md"),
    "s3_step9":  _load_skill("step09_suds_post.md"),
    "s3_step10":  _load_skill("step10_writing_discussion.md"),
    "s3_step11": _load_skill("step11_closing.md"),
}

STEP_LABELS_S3 = {
    "s3_step1":  "Step 1/11  — Welcome back",
    "s3_step2":  "Step 2/11  — Questionnaire readiness",
    "s3_step3":  "Step 3/11  — Score discussion",
    "s3_step4":  "Step 4/11  — Between-session check-in",
    "s3_step5":  "Step 5/11  — Narrative feedback",
    "s3_step6":  "Step 6/11  — Writing instructions",
    "s3_step7":  "Step 7/11  — Pre-writing SUDs",
    "s3_step8":  "Step 8/11  — Writing task",
    "s3_step9":  "Step 9/11  — Post-writing SUDs",
    "s3_step10": "Step 10/11 — Writing discussion",
    "s3_step11": "Step 11/11 — Closing",
}
...
def make_judge(step_name):
    ...
    def judge_fn(state):
        ...
        if step_name == "s3_step7" and result.get("suds_pre"):
            try:
                updates["suds_pre"] = state.get("suds_pre", []) + [int(result["suds_pre"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s3_step9" and result.get("suds_post"):
            try:
                updates["suds_post"] = state.get("suds_post", []) + [int(result["suds_post"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s3_step8" and result.get("narrative"):
...
def step11_closing(state):
    ...
    updates = {
        "current_step": "s3_step11_done",
        "session_complete": True,
        "messages": [_ai_msg(content, STEP_LABELS_S3.get("s3_step11", "Closing"))],
...
STEP_DEFS = [
    ...
    ("s3_step10",  make_prompt("s3_step10"),  make_judge("s3_step10")),
]
...
def build_session3():
    ...
    g.add_node("s3_step11_closing", step11_closing)
    ...
    for i, (name, _, _) in enumerate(STEP_DEFS):
        ...
        nxt = f"{STEP_DEFS[i+1][0]}_prompt" if i < len(STEP_DEFS) - 1 else "s3_step11_closing"
    ...
    g.add_edge("s3_step11_closing", END)
...
def create_app_s3():
    ...
    app = graph.compile(
        ...
        interrupt_before=["s3_step3_prompt"])
    return app, db
