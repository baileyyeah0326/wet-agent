...
SKILLS_S4 = {
    ...
    "s4_step2":  _load_skill("step02_questionnaire_discussion.md"),
    "s4_step3":  _load_skill("step03_checkin.md"),
    "s4_step4":  _load_skill("step04_narrative_feedback.md"),
    "s4_step5":  _load_skill("step05_writing_instructions.md"),
    "s4_step6":  _load_skill("step06_suds_pre.md"),
    "s4_step7":  _load_skill("step07_writing_task.md"),
    "s4_step8":  _load_skill("step08_suds_post.md"),
    "s4_step9":  _load_skill("step09_writing_discussion.md"),
    "s4_step10": _load_skill("step10_closing.md"),
}

STEP_LABELS_S4 = {
    "s4_step1":  "Step 1/10  — Welcome back",
    "s4_step2":  "Step 2/10  — Score discussion",
    "s4_step3":  "Step 3/10  — Between-session check-in",
    "s4_step4":  "Step 4/10  — Narrative feedback",
    "s4_step5":  "Step 5/10  — Writing instructions",
    "s4_step6":  "Step 6/10  — Pre-writing SUDs",
    "s4_step7":  "Step 7/10  — Writing task",
    "s4_step8":  "Step 8/10  — Post-writing SUDs",
    "s4_step9":  "Step 9/10  — Writing discussion",
    "s4_step10": "Step 10/10 — Closing",
}
...
def make_judge(step_name):
    ...
    def judge_fn(state):
        ...
        if step_name == "s4_step6" and result.get("suds_pre"):
            try:
                updates["suds_pre"] = state.get("suds_pre", []) + [int(result["suds_pre"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s4_step8" and result.get("suds_post"):
            try:
                updates["suds_post"] = state.get("suds_post", []) + [int(result["suds_post"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s4_step7" and result.get("narrative"):
...
def step10_closing(state):
    ...
    updates = {
        "current_step": "s4_step10_done",
        "session_complete": True,
        "messages": [_ai_msg(content, STEP_LABELS_S4.get("s4_step10", "Closing"))],
...
def build_session4():
    ...
    g.add_node("s4_step10_closing", step10_closing)
    ...
    for i, (name, _, _) in enumerate(STEP_DEFS):
        ...
        nxt = f"{STEP_DEFS[i+1][0]}_prompt" if i < len(STEP_DEFS) - 1 else "s4_step10_closing"
    ...
    g.add_edge("s4_step10_closing", END)
