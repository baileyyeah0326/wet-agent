"""
WET Agent — Session 5 (Third Writing Session)
...
Same architecture as Session 1. Key differences:
  ...
  - Step 5: Session 5 writing instructions (most upsetting part + life changes)
"""
...
SKILLS_S5 = {
    "s5_step1":  _load_skill("step01_welcome.md"),
    "s5_step2": _load_skill("step02_qready.md"),
    "s5_step3":  _load_skill("step03_questionnaire_discussion.md"),
    "s5_step4":  _load_skill("step04_checkin.md"),
    "s5_step5":  _load_skill("step05_narrative_feedback.md"),
    "s5_step6":  _load_skill("step06_writing_instructions.md"),
    "s5_step7":  _load_skill("step07_suds_pre.md"),
    "s5_step8":  _load_skill("step08_writing_task.md"),
    "s5_step9":  _load_skill("step09_suds_post.md"),
    "s5_step10":  _load_skill("step10_writing_discussion.md"),
    "s5_step11": _load_skill("step11_closing.md"),
}

STEP_LABELS_S5 = {
    "s5_step1":  "Step 1/11  — Welcome back",
    "s5_step2":  "Step 2/11  — Questionnaire readiness",
    "s5_step3":  "Step 3/11  — Score discussion",
    "s5_step4":  "Step 4/11  — Between-session check-in",
    "s5_step5":  "Step 5/11  — Narrative feedback",
    "s5_step6":  "Step 6/11  — Writing instructions",
    "s5_step7":  "Step 7/11  — Pre-writing SUDs",
    "s5_step8":  "Step 8/11  — Writing task",
    "s5_step9":  "Step 9/11  — Post-writing SUDs",
    "s5_step10": "Step 10/11 — Writing discussion",
    "s5_step11": "Step 11/11 — Closing",
}


# ═══════════════════════════════════════════════════════
# make_prompt / make_judge (Session 5)
# ═══════════════════════════════════════════════════════

def _parse_extract_spec(step_name):
    raw = SKILLS_S5[step_name].get("data_to_extract", "")
...
def make_prompt(step_name):
    def prompt_fn(state):
        if state.get("current_step") != step_name:
            ...
            label = STEP_LABELS_S5.get(step_name, step_name)
...
def make_judge(step_name):
    ...
    def judge_fn(state):
        ...
        if step_name == "s5_step7" and result.get("suds_pre"):
            try:
                updates["suds_pre"] = state.get("suds_pre", []) + [int(result["suds_pre"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s5_step9" and result.get("suds_post"):
            try:
                updates["suds_post"] = state.get("suds_post", []) + [int(result["suds_post"])]
            except (ValueError, TypeError):
                pass

        if step_name == "s5_step8" and result.get("narrative"):
...
def step11_closing(state):
    ...
    summary = _llm_json(state, (
        "[TASK] Generate clinical summary for Session 5.\n"
    ...
    ), {"summary": "Session 5 completed.", "next_priorities": []})
    ...
    updates = {
        "current_step": "s5_step11_done",
        "session_complete": True,
        "messages": [_ai_msg(content, STEP_LABELS_S5.get("s5_step11", "Closing"))],
        "session_summaries": state.get("session_summaries", []) + [session_summary],
        "clinical_observations": _add_obs(state, "session5_complete", summary.get("summary", "")),
    }
    ...
    if db is not None:
        ...
        db.add_observation(pid, session_num, "session5_complete", summary.get("summary", ""))
        db.update_patient(pid, current_session=session_num + 1, treatment_complete=True)
...
STEP_DEFS = [
    ("s5_step1",  make_prompt("s5_step1"),  make_judge("s5_step1")),
    ("s5_step2", make_prompt("s5_step2"), make_judge("s5_step2")),
    ("s5_step3",  make_prompt("s5_step3"),  make_judge("s5_step3")),
    ("s5_step4",  make_prompt("s5_step4"),  make_judge("s5_step4")),
    ("s5_step5",  make_prompt("s5_step5"),  make_judge("s5_step5")),
    ("s5_step6",  make_prompt("s5_step6"),  make_judge("s5_step6")),
    ("s5_step7",  make_prompt("s5_step7"),  make_judge("s5_step7")),
    ("s5_step8",  make_prompt("s5_step8"),  make_judge("s5_step8")),
    ("s5_step9",  make_prompt("s5_step9"),  make_judge("s5_step9")),
    ("s5_step10",  make_prompt("s5_step10"),  make_judge("s5_step10")),
]
...
def build_session5():
    ...
    g.add_node("s5_step11_closing", step11_closing)
    ...
    g.add_edge(START, "s5_step1_prompt")

    for i, (name, _, _) in enumerate(STEP_DEFS):
        g.add_edge(f"{name}_prompt", f"{name}_judge")
        nxt = f"{STEP_DEFS[i+1][0]}_prompt" if i < len(STEP_DEFS) - 1 else "s5_step11_closing"
    ...
    g.add_edge("s5_step11_closing", END)

    for qk in SAFETY_QS:
        g.add_edge(f"safety_{qk}_prompt", f"safety_{qk}_judge")

    for qk in SAFETY_QS:
        router = SAFETY_ROUTERS_S5[qk]
        possible = set()
    ...
    def _safety_result_router(state):
        if state.get("current_step") == "safety_stop":
            return "safety_stop"
        return f"{state.get('safety_return_step', 's5_step1')}_prompt"
...
def start_session5(app, db, pid):
    config = {"configurable": {"thread_id": f"patient_{pid}_s5"}}
    ...
    return app.invoke({
        "current_session": 5, "current_step": "", "session_complete": False,
...
def run_turn_s5(app, pid, message):
    config = {"configurable": {"thread_id": f"patient_{pid}_s5"}}
